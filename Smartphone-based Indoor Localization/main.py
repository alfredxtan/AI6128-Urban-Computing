...
if __name__ == "__main__":
    ...
    path_data_dir = floor_data_dir + '/path_data_files'
    path_filenames = list(Path(path_data_dir).resolve().glob("*.txt"))
    path_filenames = './data/site1/B1/path_data_files/5dda14a2c5b77e0006b17533.txt'
    
    #mwi_datas = calibrate_magnetic_wifi_ibeacon_to_position(path_filenames)
    path_datas=read_data_file(path_filenames)
    acce_datas = path_datas.acce
    magn_datas = path_datas.magn
    ahrs_datas = path_datas.ahrs
    wifi_datas = path_datas.wifi
    ...
    print(acce_datas.size, magn_datas.size, posi_datas.size, wifi_datas.size)


    total_magnetic = 0
    total_wifi = 0
    for key, value in mwi_datas.items():
        total_magnetic += value['magnetic'].shape[0]
    print(f'The total number of magnetic data is {total_magnetic}')